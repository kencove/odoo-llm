...
def pre_init_hook(cr):
    ...
    try:
        if not _column_exists(cr, table, column):
            _logger.info("[LLM_TOOL] Adding %s.%s column before install", table, column)
            cr.execute(f"ALTER TABLE {table} ADD COLUMN {column} VARCHAR")
            _logger.info("[LLM_TOOL] Column added: %s.%s", table, column)
        else:
            _logger.info("[LLM_TOOL] Column already exists: %s.%s", table, column)
        index_name = f"{table}_{column}_idx"
        _logger.info("[LLM_TOOL] Ensuring index %s on %s(%s)", index_name, table, column)
        cr.execute(f"CREATE INDEX IF NOT EXISTS {index_name} ON {table} ({column})")
        _logger.info("[LLM_TOOL] Index ensured: %s", index_name)
    except Exception:
        _logger.exception(
            "[LLM_TOOL] pre_init_hook failed while preparing %s.%s", table, column
        )
